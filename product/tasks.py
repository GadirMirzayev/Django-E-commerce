...
def send_emails_to_subscribers():
  ...
  products = Product.objects.filter(is_published=True, created_at__range=[previous_week, today]).annotate(Count('reviews__text')).order_by('-reviews__text__count')[:3]
  context = {  
      'products':products,
...
def send_emails_to_last_login30():
  ...
  today = datetime.date.today()
  products = Product.objects.filter(is_published=True, created_at__range=[previous_week, today]).annotate(Count('reviews__text')).order_by('-reviews__text__count')[:5]
  context = {  
      'products':products,
      'SITE_ADRESS': settings.SITE_ADDRESS,
    }
  body = render_to_string('emails/email-subscribers.html', context)    
  mail = EmailMessage(subject, to= users, from_email=settings.EMAIL_HOST_USER, body=body)
  mail.content_subtype='html'
  mail.send()
